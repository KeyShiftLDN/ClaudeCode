    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Execute complete bug bounty reconnaissance workflow: subdomain discovery, DNS resolution, port scanning, HTTP probing, crawling, and vulnerability scanning",
                inputSchema: {
                    type: "object",
                    properties: {
                        domain: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "naabu": {
                const { hosts, ports, topPorts } = args;
            ...
            case "httpx": {
                const { urls, followRedirects, screenshot } = args;
            ...
            case "nuclei": {
                const { targets, templates, severity } = args;
            ...
            case "bug_bounty_workflow": {
                const { domain, portScan, crawl, vulnerabilityScan, severityFilter, maxCrawlUrls, maxScanUrls, maxTopPorts, batchSize, delayBetweenBatches, crawlDepth, } = args;
